...
// static
const int TreeTableHeader::kHorizontalPadding = 7;
// static
const int TreeTableHeader::kSortIndicatorWidth = kSortIndicatorSize +
    TreeTableHeader::kHorizontalPadding * 2;

typedef std::vector<TreeTableView::VisibleColumn> Columns;

int TreeTableColumnAlignmentToCanvasAlignment(
	ui::TreeTableColumn::Alignment alignment) {
		switch (alignment) {
	case ui::TreeTableColumn::LEFT:
		return gfx::Canvas::TEXT_ALIGN_LEFT;
	case ui::TreeTableColumn::CENTER:
		return gfx::Canvas::TEXT_ALIGN_CENTER;
	case ui::TreeTableColumn::RIGHT:
...
static int GetClosestVisibleColumnIndex(const TreeTableView* table, int x) {
	const std::vector<TreeTableView::VisibleColumn>& columns(
...
TreeTableHeader::TreeTableHeader(TreeTableView* table) : table_(table) {
...
void TreeTableHeader::Layout() {
  SetBounds(x(), y(), table_->width(), GetPreferredSize().height());
}

void TreeTableHeader::OnPaint(gfx::Canvas* canvas) {
  ...
  for (size_t i = 0; i < columns.size(); ++i) {
    ...
    if (paint_sort_indicator &&
        columns[i].column.alignment == ui::TreeTableColumn::RIGHT) {
    ...
    canvas->DrawStringInt(
        ...
        TreeTableColumnAlignmentToCanvasAlignment(columns[i].column.alignment));
    ...
    if (paint_sort_indicator) {
      ...
      int indicator_x = 0;
      ui::TreeTableColumn::Alignment alignment = columns[i].column.alignment;
      if (base::i18n::IsRTL()) {
        if (alignment == ui::TreeTableColumn::LEFT)
          alignment = ui::TreeTableColumn::RIGHT;
        else if (alignment == ui::TreeTableColumn::RIGHT)
          alignment = ui::TreeTableColumn::LEFT;
      }
      switch (alignment) {
        case ui::TreeTableColumn::LEFT:
...
gfx::Size TreeTableHeader::GetPreferredSize() {
  return gfx::Size(1, kVerticalPadding * 2 + font_.GetHeight());
}

gfx::NativeCursor TreeTableHeader::GetCursor(const ui::MouseEvent& event) {
  return GetResizeColumn(GetMirroredXInView(event.x())) != -1 ?
      GetResizeCursor() : View::GetCursor(event);
}

bool TreeTableHeader::OnMousePressed(const ui::MouseEvent& event) {
...
bool TreeTableHeader::OnMouseDragged(const ui::MouseEvent& event) {
  if (is_resizing()) {
    const int scale = base::i18n::IsRTL() ? -1 : 1;
...
void TreeTableHeader::OnMouseReleased(const ui::MouseEvent& event) {
  ...
  if (!was_resizing && event.IsOnlyLeftMouseButton() &&
    ...
    const TreeTableView::VisibleColumn& column(table_->visible_columns()[index]);
...
void TreeTableHeader::OnMouseCaptureLost() {
  if (is_resizing()) {
...
int TreeTableHeader::GetResizeColumn(int x) const {
  const Columns& columns(table_->visible_columns());
  if (columns.empty())
    return -1;
