...
class TreeTableNodeIterator {
 public:
  ...
  TreeTableNodeIterator(NodeType* node, bool (*prune)(NodeType*))
      : prune_(prune) {
    int index = 0;
  ...
  explicit TreeTableNodeIterator(NodeType* node) : prune_(NULL) {
    if (!node->empty())
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(TreeTableNodeIterator);
};
