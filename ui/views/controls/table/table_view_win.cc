...
// static
const int TableViewWin::kImageSize = 18;

TableViewWin::TableViewWin(ui::TableModel* model,
...
void TableViewWin::SetModel(ui::TableModel* model) {
  if (model == model_)
    return;
...
View* TableViewWin::CreateParentIfNecessary() {
  return this;
}

void TableViewWin::SetSortDescriptors(const SortDescriptors& sort_descriptors) {
...
int TableViewWin::RowCount() const {
  if (!list_view_)
    return 0;
  return ListView_GetItemCount(list_view_);
}

int TableViewWin::SelectedRowCount() {
  if (!list_view_)
    return 0;
  return ListView_GetSelectedCount(list_view_);
}

void TableViewWin::Select(int model_row) {
  if (!list_view_)
    return;
...
void TableViewWin::SetSelectedState(int model_row, bool state) {
  if (!list_view_)
    return;
...
void TableViewWin::SetFocusOnItem(int model_row) {
  if (!list_view_)
    return;
...
int TableViewWin::FirstSelectedRow() {
  if (!list_view_)
    return -1;
...
bool TableViewWin::IsItemSelected(int model_row) {
  if (!list_view_)
    return false;
...
bool TableViewWin::ItemHasTheFocus(int model_row) {
  if (!list_view_)
    return false;
...
TableViewWin::iterator TableViewWin::SelectionBegin() {
	return TableViewWin::iterator(this, LastSelectedViewIndex());
}

TableViewWin::iterator TableViewWin::SelectionEnd() {
	return TableViewWin::iterator(this, -1);
}

void TableViewWin::OnItemsChanged(int start, int length) {
  if (!list_view_)
    return;
...
void TableViewWin::OnModelChanged() {
  if (!list_view_)
    return;
...
void TableViewWin::OnItemsAdded(int start, int length) {
  if (!list_view_)
    return;
...
void TableViewWin::OnItemsRemoved(int start, int length) {
  if (!list_view_)
    return;
...
void TableViewWin::AddColumn(const ui::TableColumn& col) {
  DCHECK_EQ(0u, all_columns_.count(col.id));
  all_columns_[col.id] = col;
}

void TableViewWin::SetColumns(const std::vector<ui::TableColumn>& columns) {
...
void TableViewWin::OnColumnsChanged() {
  column_count_ = static_cast<int>(visible_columns_.size());
  ResetColumnSizes();
}

void TableViewWin::SetColumnVisibility(int id, bool is_visible) {
  bool changed = false;
...
void TableViewWin::SetVisibleColumns(const std::vector<int>& columns) {
...
bool TableViewWin::IsColumnVisible(int id) const {
...
const ui::TableColumn& TableViewWin::GetColumnAtPosition(int pos) {
  return all_columns_[visible_columns_[pos]];
}

bool TableViewWin::HasColumn(int id) {
  return all_columns_.count(id) > 0;
}

gfx::Point TableViewWin::GetKeyboardContextMenuLocation() {
...
void TableViewWin::SetCustomColorsEnabled(bool custom_colors_enabled) {
  custom_colors_enabled_ = custom_colors_enabled;
}

bool TableViewWin::GetCellColors(int model_row,
                              int column,
...
// static
LRESULT CALLBACK TableViewWin::TableWndProc(HWND window,
                                         UINT message,
                                         WPARAM w_param,
                                         LPARAM l_param) {
	TableViewWin* table_view = reinterpret_cast<TableViewWrapper*>(
  ...
  switch (message) {
    ...
    case WM_KEYDOWN: {
		MSG msg = { table_view->GetNativeControlHWND(), message, w_param, l_param, GetMessageTime() };
		ui::KeyEvent key(msg, message == WM_CHAR);
		views::FocusManager * focus_manager = table_view->GetFocusManager();
		if (focus_manager&&focus_manager->IsTabTraversalKeyEvent(key)) {
    ...
    case WM_MOUSEMOVE: {
      if (in_mouse_down) {
		...
		if (View::ExceededDragThreshold(gfx::Vector2d(x - mouse_down_x, y - mouse_down_y))) {
...
LRESULT CALLBACK TableViewWin::TableHeaderWndProc(HWND window, UINT message,
...
HWND TableViewWin::CreateNativeControl(HWND parent_container) {
  ...
  if (table_type_ == ICON_AND_TEXT) {
	...
	gfx::Canvas canvas(gfx::Size(kImageSize, kImageSize), ui::SCALE_FACTOR_100P, false);
    {
      base::win::ScopedHICON empty_icon(
		  IconUtil::CreateHICONFromSkBitmap(canvas.ExtractImageRep().sk_bitmap()));
  ...
  if (!resizable_columns_) {
    ...
    header_original_handler_ = ui::SetWindowProc(header,
		&TableViewWin::TableHeaderWndProc);
  }

  SetWindowLongPtr(list_view_, GWLP_USERDATA,
      reinterpret_cast<LONG_PTR>(&table_view_wrapper_));
  original_handler_ =
	  ui::SetWindowProc(list_view_, &TableViewWin::TableWndProc);
...
void TableViewWin::ToggleSortOrder(int column_id) {
  SortDescriptors sort = sort_descriptors();
...
void TableViewWin::UpdateItemsLParams(int start, int length) {
  LVITEM item;
...
void TableViewWin::SortItemsAndUpdateMapping() {
  if (!is_sorted()) {
	  ListView_SortItems(list_view_, &TableViewWin::NaturalSortFunc, this);
	view_to_model_.clear();
	model_to_view_.clear();
    return;
  }
  ...
  // Sort the items.
  ListView_SortItems(list_view_, &TableViewWin::SortFunc, this);
...
bool TableViewWin::SelectMultiple(int view_index, int mark_view_index) {
  int group_id = 0;
  if (model_->HasGroups()) {
...
// static
int CALLBACK TableViewWin::SortFunc(LPARAM model_index_1_p,
  ...
  TableViewWin* table_view = reinterpret_cast<TableViewWin*>(table_view_param);
  return table_view->CompareRows(model_index_1, model_index_2);
}

// static
int CALLBACK TableViewWin::NaturalSortFunc(LPARAM model_index_1_p,
                                        LPARAM model_index_2_p,
                                        LPARAM table_view_param) {
  return model_index_1_p - model_index_2_p;
}

void TableViewWin::ResetColumnSortImage(int column_id, SortDirection direction) {
...
void TableViewWin::InsertColumn(const ui::TableColumn& tc, int index) {
  if (!list_view_)
    return;
...
LRESULT TableViewWin::OnNotify(int w_param, LPNMHDR hdr) {
  if (!model_)
    return 0;
  ...
  switch (hdr->code) {
    ...
    case LVN_GETINFOTIP: {
      ...
      if (tooltip.length() >= static_cast<size_t>(info_tip->cchTextMax)) {
        ...
        const base::char16 kEllipsis = 0x2026;
...
int TableViewWin::CompareRows(int model_row1, int model_row2) {
  if (model_->HasGroups()) {
...
int TableViewWin::GetColumnWidth(int column_id) {
  if (!list_view_)
    return -1;
...
void TableViewWin::PaintAltText() {
  if (alt_text_.empty())
    return;
...
LRESULT TableViewWin::OnCustomDraw(NMLVCUSTOMDRAW* draw_info) {
          ...
          if (ListView_GetItemRect(list_view_, view_index, &icon_rect,
            ...
            if (IntersectRect(&intersection, &icon_rect, &client_rect)) {
              gfx::Canvas canvas(gfx::Size(icon_rect.right - icon_rect.left,
				  icon_rect.bottom - icon_rect.top),ui::SCALE_FACTOR_100P,false);
...
void TableViewWin::UpdateListViewCache(int start, int length, bool add) {
  ignore_listview_change_ = true;
  UpdateListViewCache0(start, length, add);
  ignore_listview_change_ = false;
}

void TableViewWin::ResetColumnSizes() {
  if (!list_view_)
    return;
...
gfx::Size TableViewWin::GetPreferredSize() {
  return preferred_size_;
}

void TableViewWin::SetPreferredSize(const gfx::Size& size) {
  preferred_size_ = size;
  PreferredSizeChanged();
}

int TableViewWin::ModelToView(int model_index) const {
  if (model_to_view_.empty())
    return model_index;
...
int TableViewWin::ViewToModel(int view_index) const {
  if (view_to_model_.empty())
    return view_index;
...
void TableViewWin::SetAltText(const base::string16& alt_text) {
...
void TableViewWin::UpdateListViewCache0(int start, int length, bool add) {
  if (is_sorted()) {
    if (add)
  ...
  for (int j = 0; j < column_count_; ++j) {
    ...
    for (int i = start; i < start + length; ++i) {
      ...
      item.iSubItem = j;
      base::string16 text = model_->GetText(i, visible_columns_[j]);
...
void TableViewWin::OnDoubleClick() {
  if (!ignore_listview_change_ && table_view_observer_) {
    table_view_observer_->OnDoubleClick();
  }
}

void TableViewWin::OnMiddleClick() {
  if (!ignore_listview_change_ && table_view_observer_)
    table_view_observer_->OnMiddleClick();
}

void TableViewWin::OnSelectedStateChanged() {
  if (!ignore_listview_change_ && table_view_observer_) {
    table_view_observer_->OnSelectionChanged();
  }
}

bool TableViewWin::OnKeyDown(ui::KeyboardCode virtual_keycode) {
  if (!ignore_listview_change_ && table_view_observer_) {
...
void TableViewWin::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (!list_view_)
    return;
  SendMessage(list_view_, WM_SETREDRAW, static_cast<WPARAM>(FALSE), 0);
  Layout();
...
int TableViewWin::PreviousSelectedViewIndex(int view_index) {
...
int TableViewWin::LastSelectedViewIndex() {
  return PreviousSelectedViewIndex(RowCount());
}

void TableViewWin::UpdateContentOffset() {
  content_offset_ = 0;
...
void TableViewWin::UpdateGroups() {
  // Add the groups.
...
gfx::Rect TableViewWin::GetAltTextBounds() {
...
gfx::Font TableViewWin::GetAltTextFont() {
  return ResourceBundle::GetSharedInstance().GetFont(ResourceBundle::BaseFont);
}

void TableViewWin::VisibilityChanged(View* starting_from, bool is_visible) {
...
//
TableSelectionIterator::TableSelectionIterator(TableViewWin* view,
                                               int view_index)
    : table_view_(view),
