...
void RectangleString::AddWord(const string16& word) {
  if (word.length() < max_cols_) {
	...
	{		
		if (current_col_ + (char_pos - char_start) >= max_cols_) {
			Append(word.substr(array_start, array_pos - array_start));
			NewLine(true);
			array_start = array_pos;
			char_start = char_pos;
			array_pos = char_start;
		}
		char_pos ++;
	} 
    // Add the last remaining fragment, if any.
    if (array_start != array_pos)
      Append(word.substr(array_start, array_pos - array_start));
  }
}
