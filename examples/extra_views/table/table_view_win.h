...
#ifndef NATIVE_CONTROLS_TABLE_TABLE_VIEW_WIN_H_
#define NATIVE_CONTROLS_TABLE_TABLE_VIEW_WIN_H_
#pragma once
...
class EXTRA_EXPORT TableSelectionIterator {
 public:
  TableSelectionIterator(TableViewWin* view, int view_index);
...
class EXTRA_EXPORT TableViewWin : public NativeControl,
 ...
 public:
  ...
  TableViewWin(ui::TableModel* model, const std::vector<ui::TableColumn>& columns,
            TableTypes table_type, bool single_selection,
            bool resizable_columns, bool autosize_columns);
  virtual ~TableViewWin();
  ...
  void SetAltText(const base::string16& alt_text);
 ...
 private:
  ...
  struct TableViewWrapper {
	  explicit TableViewWrapper(TableViewWin* view) : table_view(view) { }
	  TableViewWin* table_view;
  };
  ...
  base::string16 alt_text_;

  DISALLOW_COPY_AND_ASSIGN(TableViewWin);
};
#endif  // defined(OS_WIN)

}  // namespace views

#endif  // NATIVE_CONTROLS_TABLE_TABLE_VIEW_WIN_H_
